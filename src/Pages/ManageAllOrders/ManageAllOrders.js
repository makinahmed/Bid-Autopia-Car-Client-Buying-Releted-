...
const ManageAllOrders = () => {
  ...
  useEffect(() => {
    fetch(`https://bid-autopia-server-makinahmed.vercel.app/manageAllOrders`)
      .then((res) => res.json())
      .then((data) => {
        setOrders(data);
      });
  ...
  const handleOnclick = (email, id) => {
    ...
    fetch(`https://bid-autopia-server-makinahmed.vercel.app/approve`, {
      method: "PUT",
      headers: {
        "content-type": "application/json",
      },
          ...
          <tbody className="text-center">
            {orders?.map((order) => (
              <tr key={order._id}>
                ...
                <td>
                  <button
                    className="btn btn-info fs-5 mx-auto"
                    onClick={() => handleOnclick(order?.email, order?._id)}
                  >
